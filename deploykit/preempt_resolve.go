...
// servable off a plain map[string]spec.DeployNode: the shape both a freshly-loaded uf.Deploy
...
// seam is DELETED) already carry — spec.DeployNode is a type alias for
...
// per-field via MergeDeployNode. project may be nil/empty (no project loaded, e.g. a project-less
...
// loaderkit.LoadHostDeployConfigViaExecutor; a host in-proc caller may pass a LoadDeployConfig
...
func MergedDeployTree(project map[string]spec.DeployNode, context string, read func() (*DeployConfig, error)) map[string]spec.DeployNode {
	out := make(map[string]spec.DeployNode, len(project))
	...
	if dc != nil {
		for name, node := range dc.Deploy {
			out[name] = MergeDeployNode(out[name], node)
		}
	}
	return out
}
...
func FilterPreemptibleHolders(tree map[string]spec.DeployNode) []spec.HolderDescriptor {
