...
func EmitDownload(b *strings.Builder, t vmshared.Op, img *buildkit.ResolvedBox) error {
	...
	//
	// The cache lives under /var/cache/charly, NOT /tmp (RCA 2026-09-24): a
	// BuildKit cache mount rooted under /tmp combined with a nested container
	// runtime writing under /tmp in the same RUN makes BuildKit materialise
	// /tmp at the process umask (0755) instead of preserving the lower layer's
	// 1777, breaking every uid-1000 writer of /tmp (supervisord's logfile,
	// pod-dbus's socket). /var/cache is the correct home for build caches.
