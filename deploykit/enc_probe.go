...
// via the SAME deploykit.LoadDeployConfig every deploy-config consumer already uses), so they
...
// but takes an ALREADY-LOADED dc instead of reaching the package-level LoadDeployConfig()
...
// out-of-process loads dc itself via loaderkit.LoadHostDeployConfigViaExecutor (over the
...
func EncPlanForConfig(dc *DeployConfig, boxName, instance, volume, scopeDir string) ([]spec.EncVolumePlan, error) {
...
// LoadDeployConfig's own nil-on-absent contract). See EncPlanForConfig's doc comment for
// the placement-dependency rationale a caller of this variant is avoiding.
func LoadEncryptedVolumeFromConfig(dc *DeployConfig, boxName, instance string) ([]vmshared.DeployVolumeConfig, string, error) {
...
func EncStatusFromConfig(dc *DeployConfig, boxName, instance string) error {
	mounts, storagePath, err := LoadEncryptedVolumeFromConfig(dc, boxName, instance)
	if err != nil {
		return err
	}
