...
// extraction). WriteLabels (write_labels.go) is the FORMAT-only emitter; ExtractMetadata is its
// deploy-side mirror — both consume/produce the SAME ai.opencharly.* wire contract (sdk/spec's
// label_consts.go), so they live together. ExtractMetadata is a PURE function of (engine,
// imageRef) — no *Config, no candy graph — used by build, deploy, and check alike (charly/labels.go
// previously carried it as the sole holdout after K3 already dissolved everything else in that
// file: the label constants → spec, InspectLabels → kit.InspectImageLabels).

// InspectLabels reads OCI labels from a local image via engine inspect. Package-level var for
// testability (mocked by callers that need to exercise ExtractMetadata's decode logic without a
// real container store).
