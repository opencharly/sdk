...
// invariant (`charly deploy from-box`) depends on this list being complete.
//
...
var CapabilityLabelMap = map[string]string{
	...
	// moved to DeployNode in schema v4 (deployment choices, no
	...
	// Engine moved to DeployNode in schema v4 (deploy-host choice).
	"Init":         spec.LabelInit,
	...
	// inspect` and `charly deploy from-box`; the Deploy section is
...
// AcmeEmail / Engine moved to DeployNode". The fields stay on spec.BoxMetadata because
...
// CapabilitiesFromLabels is the source-less loader used by `charly deploy from-box`:
