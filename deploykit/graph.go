...
// This is the build-mode candy/box dependency-graph subsystem, relocated from
// charly core into sdk/deploykit (P8) so the render engine (which resolves candy
// install order) lives outside charly. It operates over deploykit.CandyModel and
// buildkit.ResolvedBox. Behaviour is byte-identical to the former charly/graph.go
...
func ExpandCandy(requested []string, layers map[string]CandyModel) ([]string, error) {
	...
	expand = func(rawName string) error {
		// BareRef-normalize every ref before lookup so callers that pass the RAW
		// charly.yml candy list resolve against the BareRef-keyed candy map. The
		// single chokepoint every ResolveCandyOrder caller funnels through;
		// idempotent for already-bare names, so the build/install path is
		// byte-unchanged.
		...
		if len(layer.GetIncludedCandy()) > 0 {
			expanding[name] = true
			for _, included := range layer.GetIncludedCandy() {
...
//
// Self-refs and refs to boxes not in the local map are filtered out. This covers
// both an empty Base from a resolved `from:` box and namespaced remote bases.
//
...
func topoSort(graph map[string][]string) ([]string, error) {
	...
	for len(queue) > 0 {
		...
		slices.Sort(dependents) // for determinism
		for _, dependent := range dependents {
			inDegree[dependent]--
			if inDegree[dependent] == 0 {
				queue = append(queue, dependent)
				slices.Sort(queue) // maintain sorted order
			}
		}
	}
