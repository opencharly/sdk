...
// MutateDeployConfig and asserts every entry survives.
...
func (f *fakeOverlayStore) read() (*DeployConfig, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.state == nil {
		return nil, nil
	...
	cp := &DeployConfig{Deploy: make(map[string]DeployNode, len(f.state.Deploy))}
	for k, v := range f.state.Deploy {
		cp.Deploy[k] = v
	}
	return cp, nil
}

func (f *fakeOverlayStore) save(dc *DeployConfig) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.state = dc
	return nil
...
// MutateDeployConfig derives from it is a real, isolated OS lock — never the operator's own.
func isolateDeployConfigPath(t *testing.T) {
	t.Helper()
	t.Setenv(kit.DeployConfigEnv, filepath.Join(t.TempDir(), "charly.yml"))
}

// TestMutateDeployConfig_ConcurrentMutatorsOfDifferentEntriesAllSurvive is the GREEN arm: N
...
func TestMutateDeployConfig_ConcurrentMutatorsOfDifferentEntriesAllSurvive(t *testing.T) {
	...
	for i, err := range errs {
		if err != nil {
			t.Fatalf("MutateDeployConfig() writer %d error = %v", i, err)
		}
	}
	if store.state == nil {
		t.Fatal("no config was written at all")
	}
	if len(store.state.Deploy) != n {
		t.Fatalf("overlay has %d entries after %d concurrent writers, want %d — a write was lost: %v", len(store.state.Deploy), n, n, store.state.Deploy)
	}
	for i := range n {
		name := fmt.Sprintf("bed-%02d", i)
		entry, ok := store.state.Deploy[name]
		if !ok {
...
func TestUnlockedReadModifySaveLosesUpdates(t *testing.T) {
	...
	if got := len(store.state.Deploy); got != 1 {
