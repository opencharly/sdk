...
// config-setup writes, candy/plugin-bundle's import/reset/ephemeral writes): read the overlay,
...
// MutateBundleConfig and asserts every entry survives.
...
func (f *fakeOverlayStore) read() (*BundleConfig, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.state == nil {
		return nil, nil
	...
	cp := &BundleConfig{Bundle: make(map[string]BundleNode, len(f.state.Bundle))}
	for k, v := range f.state.Bundle {
		cp.Bundle[k] = v
	}
	return cp, nil
}

func (f *fakeOverlayStore) save(dc *BundleConfig) error {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.state = dc
	return nil
...
// MutateBundleConfig derives from it is a real, isolated OS lock — never the operator's own.
func isolateDeployConfigPath(t *testing.T) {
	t.Helper()
	t.Setenv(kit.DeployConfigEnv, filepath.Join(t.TempDir(), "charly.yml"))
}

// TestMutateBundleConfig_ConcurrentMutatorsOfDifferentEntriesAllSurvive is the GREEN arm: N
...
func TestMutateBundleConfig_ConcurrentMutatorsOfDifferentEntriesAllSurvive(t *testing.T) {
	...
	for i := range n {
		...
		go func(i int) {
			...
			_, errs[i] = MutateBundleConfig(store.read, store.save, func(dc *BundleConfig) (bool, error) {
				dc.Bundle[name] = BundleNode{Image: name, ResolvedImage: name + ":latest"}
				return true, nil
			})
		}(i)
	}
	...
	for i, err := range errs {
		if err != nil {
			t.Fatalf("MutateBundleConfig() writer %d error = %v", i, err)
		}
	}
	if store.state == nil {
		t.Fatal("no config was written at all")
	}
	if len(store.state.Bundle) != n {
		t.Fatalf("overlay has %d entries after %d concurrent writers, want %d — a write was lost: %v", len(store.state.Bundle), n, n, store.state.Bundle)
	}
	for i := range n {
		name := fmt.Sprintf("bed-%02d", i)
		entry, ok := store.state.Bundle[name]
		if !ok {
...
func TestUnlockedReadModifySaveLosesUpdates(t *testing.T) {
	...
	if got := len(store.state.Bundle); got != 1 {
