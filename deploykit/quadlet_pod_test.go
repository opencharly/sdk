...
func TestGeneratePodQuadlet_ShmSize(t *testing.T) {
	cfg := QuadletConfig{
		BoxName:  "my-app",
		Network:  "charly",
		Security: vmshared.SecurityConfig{ShmSize: "1g"},
	}
	content := GeneratePodQuadlet(cfg)
...
func TestGenerateQuadlet_PodMode_NoShmSize(t *testing.T) {
	cfg := QuadletConfig{
		BoxName:    "my-app",
		ImageRef:   "ghcr.io/opencharly/my-app:latest",
		Home:       "/home/user",
...
// TestSortedSidecarEnv relocated from charly/sidecar_test.go (P11b): it exercises the
// unexported sortedSidecarEnv helper the sidecar quadlet generator uses, so it lives
// WITH the engine (R5).
