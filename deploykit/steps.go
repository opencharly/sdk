// Package deploykit holds the InstallPlan step VOCABULARY — the 13 concrete
// InstallStep implementations + their field-structs + the pure classification
// helpers — that any deploy front-end (charly's host engine, an out-of-tree
// deploy plugin) imports. Each concrete step implements spec.InstallStep (homed
// in sdk/spec with the IR enums + the InstallPlan container). deploykit imports
// spec; the aliases below let the moved code read unchanged.
package deploykit
