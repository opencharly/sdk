...
func TestResolveVmSshPortNilConfigNoPanic(t *testing.T) {
	...
	port, err := ResolveVmSshPort(context.Background(), sp, "some-vm")
	if err != nil {
...
func TestResolveVmSshPortNilSpec(t *testing.T) {
	port, err := ResolveVmSshPort(context.Background(), nil, "some-vm")
	if err != nil {
		t.Fatalf("ResolveVmSshPort(context.Background(), nil, ...) must not error: %v", err)
	}
	if port != 22 {
