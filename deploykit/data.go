...
// data.go — the runtime data-provisioning mechanism (K4 lane B: relocated from charly/data.go, a
// genuinely pure host-engine mechanism with no project-loader dependency — it takes an
// already-resolved *spec.BoxMetadata + volume lists as parameters). Homed in deploykit (not kit)
// because it needs ResolvedBindMount, a deploykit-only type (kit cannot import deploykit — that
// would cycle). Consumed directly by candy/plugin-deploy-pod (pod_lifecycle_resolve.go's move) AND
// by charly core's config_image.go (group 3, not moving yet) via the deploykit_pod_aliases.go
// passthrough. provisionData is exported (ProvisionData) since it now crosses a package boundary.
...
type seedTarget struct {
	bareName string // bare volume name (no "charly-<image>-" prefix)
	kind     seedKind
	// mountSource is passed to -v <src>:/seed. For bind: absolute host path.
	// For named: the full volume name (e.g. "charly-jupyter-workspace"). Podman
...
func ProvisionData(engine string, imageRef string, meta *spec.BoxMetadata,
	...
	// (e.g. charly-versa-ecovoyage-workspace) reduced to `ecovoyage-workspace`
...
func provisionFromRunnableImage(engine string, imageRef string, meta *spec.BoxMetadata,
	entry spec.LabelDataEntry, target seedTarget, mode DataProvisionMode) error {
...
func provisionFromScratchImageToHost(engine string, imageRef string,
	entry spec.LabelDataEntry, hostPath string) error {
	binary := kit.EngineBinary(engine)
