...
// so it stays an INJECTED save callback, mirroring SaveDeployConfig's own marshalNode-callback
// shape. The process-shared deploy-config FLOCK is NO LONGER injected: both bodies below run inside
// MutateDeployConfig (deploy_config_cycle.go), THE one locked read-modify-write cycle every overlay
...
// persisted as the entry's `vm:` cross-ref so a deploy-keyed entry (a kind:check VM bed, whose
// deploy key e.g. `check-k3s-vm` differs from `vm:<entity>`) carries the linkage teardown needs to
// find + remove it.
//
// The load→modify→save runs inside MutateDeployConfig, so it holds the process-shared blocking
...
// deploy add vm:<name>`) load → modify → save the shared ~/.config/charly/charly.yml and silently
// drop each other's entry.
//
// read is the current-state re-read that cycle performs. A nil read falls back to
// LoadDeployConfig — the DeployStateHost-backed host read — so an IN-PROCESS host caller passes nil
...
func SaveVmDeployState(deployName, vmEntity string, state *spec.VmDeployState, save func(*DeployConfig) error, read func() (*DeployConfig, error)) error {
	loadBase := read
	if loadBase == nil {
...
func RemoveVmDeployEntry(deployName string, save func(*DeployConfig) error, read func() (*DeployConfig, error)) error {
	loadBase := read
	if loadBase == nil {
		loadBase = LoadDeployConfig
	}
...
// saveVmStateInto is: MutateDeployConfig takes a function over fresh state.
func removeVmEntriesFrom(dc *DeployConfig, deployName string) bool {
