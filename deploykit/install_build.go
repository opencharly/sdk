...
func BuildDeployPlan(layer CandyModel, img *ResolvedBox, hostCtx HostContext) (*InstallPlan, error) {
	...
	if img == nil {
		return nil, fmt.Errorf("BuildDeployPlan: nil image %q", layer.GetName())
	}
	...
	plan := &InstallPlan{
		...
		Candy:           layer.GetName(),
		CandiesIncluded: []string{layer.GetName()},
	}
	...
	if layer.Reboot() {
		plan.Steps = append(plan.Steps, &RebootStep{CandyName: layer.GetName()})
	}
...
func CompileShellSnippetSteps(layer CandyModel, img *ResolvedBox, hostCtx HostContext) []InstallStep {
	...
	for _, shell := range []string{"bash", "zsh", "fish", "sh"} {
		...
		dest, useDropin := shellSnippetDestination(layer.GetName(), shell, hostCtx, snippetHome, spec.Path)
		if dest == "" {
			continue
		}
...
func compileBuilderSteps(layer CandyModel, img *ResolvedBox, hostCtx HostContext) []InstallStep {
	...
	for _, bName := range names {
		...
		if pre, ok := hostCtx.BuilderContext[BuilderCtxKey(layer.GetName(), bName)]; ok {
...
// base-only context.
func collectBuilderContext(layer CandyModel, builderName string, img *ResolvedBox, hostCtx HostContext) map[string]any {
	ctx := map[string]any{
		"layer":   layer.GetName(),
		"builder": builderName,
		"home":    img.Home,
	}
	if pre, ok := hostCtx.BuilderContext[BuilderCtxKey(layer.GetName(), builderName)]; ok {
