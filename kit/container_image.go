...
// CHECK-ENGINE cone Option A — a genuinely pure podman/docker CLI invocation charly core's
// check-endpoint reverse-leg (check_endpoint_resolve.go) reaches importing zero kit). THE single
// container→image-ref inspector — there is exactly one inspect implementation. kit re-exports
// the symbols here so every existing kit.ContainerImageRef / kit.ContainerImage call site
// (charly core + candy/plugin-deploy-pod) is untouched. New consumers should import
// spec/container directly.
