...
// refs_git.go — the remote-repo GIT primitives (P7). Pure git-exec + string parsing, shared by
// core (reconcile's --remote tag query, the collection walk) and the refs fetch backend
// (candy/plugin-refs). The FETCH ORCHESTRATION (override → cache → clone → migrate) lives in the
// plugin; these are the reusable mechanisms it composes.
