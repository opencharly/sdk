...
// applying the per-request policy carried in HTTPRequest — the leg that proves the
// out-of-process http verb dials with HOST-vantage semantics identical to compiled-in.
//
// P12a: relocated from charly/check_http_test.go.
func TestDoHTTPRequest(t *testing.T) {
	...
	t.Run("status + body + header", func(t *testing.T) {
		...
		defer srv.Close()
		resp, err := DoHTTPRequest(context.Background(), base, HTTPRequest{URL: srv.URL})
		if err != nil {
	...
	t.Run("allow_insecure against self-signed TLS", func(t *testing.T) {
		...
		resp, err := DoHTTPRequest(context.Background(), base, HTTPRequest{URL: srv.URL, AllowInsecure: true})
		if err != nil {
	...
	t.Run("custom timeout", func(t *testing.T) {
		...
		defer slow.Close()
		if _, err := DoHTTPRequest(context.Background(), base, HTTPRequest{URL: slow.URL, Timeout: "10ms"}); err == nil {
	...
	t.Run("no_follow_redirects", func(t *testing.T) {
		...
		if resp, err := DoHTTPRequest(context.Background(), base, HTTPRequest{URL: srv.URL + "/redir"}); err != nil || resp.Status != 200 {
			t.Errorf("follow: status=%d err=%v, want 200/nil", resp.Status, err)
		}
		// NoFollowRedirects: returns the 302 itself.
		resp, err := DoHTTPRequest(context.Background(), base, HTTPRequest{URL: srv.URL + "/redir", NoFollowRedirects: true})
		if err != nil {
