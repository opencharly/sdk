...
// *vmshared.DeployNode (the deploy overlay — currently unused, reserved for a future
...
func ResolveCheckVarsRuntime(meta *spec.BoxMetadata, deploy *vmshared.DeployNode, engine, deployName, containerName, instance string) (*CheckVarResolver, error) {
...
func buildTimeVars(meta *spec.BoxMetadata, instance string) map[string]string {
	env := map[string]string{}
	if meta == nil {
		return env
	}
...
func mergeRuntimeVars(env map[string]string, meta *spec.BoxMetadata, c *ContainerInspection, deployName, instance string) {
	if c == nil {
		return
	}
	...
	// uniformly — same helper deploykit's deploy-config code uses.
