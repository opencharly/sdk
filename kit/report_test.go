...
func sampleResults() []StepResult {
	return []StepResult{
		{Keyword: "check", Text: "the marker exists", Origin: "layer-a", StepID: "a-marker",
			Result: spec.CheckResult{Verb: "file", Status: StatusPass, Message: "found", Elapsed: 12 * time.Millisecond}},
		{Keyword: "check", Text: "the port is open", Origin: "layer-a", StepID: "a-port",
			Result: spec.CheckResult{Verb: "port", Status: StatusFail, Message: "connection refused", Elapsed: 5 * time.Millisecond}},
		{Keyword: "check", Text: "the gpu is present", Origin: "layer-b", StepID: "b-gpu",
			Result: spec.CheckResult{Verb: "command", Status: StatusSkip, Message: "no gpu vendor", Elapsed: time.Millisecond}},
	}
}
...
func TestRenderStep_RetryInfo(t *testing.T) {
	...
	FormatStepResultsText(&b, []StepResult{{
		...
		Result: spec.CheckResult{Status: StatusPass, Attempts: 5, TotalElapsed: 12300 * time.Millisecond},
	}})
	...
	FormatStepResultsText(&one, []StepResult{{
		...
		Result: spec.CheckResult{Status: StatusPass, Attempts: 1},
	}})
...
func TestStepResultCarriesOp(t *testing.T) {
	op := &spec.Op{}
	sr := StepResult{Result: spec.CheckResult{Op: op, Status: StatusPass}}
	if sr.Result.Op != op {
