...
// prose-only step (an agent-run/agent-check with no verb) binds to an AGENT — this
// grader. The grader spawns the configured `kind: agent` CLI on the host, hands it
// the entity's goal + the step's prose + the live target handle, lets it probe the
// running deployment with the full `charly check` surface, and parses back a
// structured pass/fail verdict.
//
// Bounded by construction: ONE agent invocation per prose step (never the plateau
// loop), wall-clock-capped, and an unparseable / failed / timed-out grader is a FAIL
// with evidence — never a silent pass. The grader is wired only by
// `charly check feature run <deployment>` (a live deployment the agent can reach);
// `charly box feature run` (disposable, no stable target) leaves it nil so prose
// steps stay advisory-skip, and the harness loop never sets it.
