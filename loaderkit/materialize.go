...
// recognized-but-unconnected external deploy substrate routes to the deploy builder, a declared-
...
func Materialize(pn spec.ParsedNode, t spec.Threaded, seams spec.MaterializeSeams, acc *spec.MaterializedProject) error {
	...
	// it routes to the deploy builder, the same path the deploy-shape kinds (pod/vm/kubernetes/local/
	...
	if t.DeploySubstrates[pn.Disc] {
		return seams.BuildDeployEntity(pn, acc)
	}
