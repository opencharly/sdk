...
func (w *walker) parseDocs(lp *spec.LoadedProject, data []byte, srcLabel, srcDir string, acc *nsAcc) (spec.ImportList, error) {
	// The per-document pipeline is the ONE loaderkit.ParseDocStream composer (parser
	// consolidation F2.5) — the file walk and the embedded-vocab stream drive the same loop.
	docs, importQueue, scanSpecs, err := ParseDocStream(data, srcLabel, srcDir, w.seams)
	if err != nil {
		return nil, err
	}
	lp.Docs = append(lp.Docs, docs...)
	acc.discoverSpecs = append(acc.discoverSpecs, scanSpecs...)
