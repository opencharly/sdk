...
// substrate-template maps (local/android after Cutover I). Relocated from
// charly/android_loader_test.go with the merge helper (K1-proper), using raw
// JSON bodies so the test carries no charly-core type dependency.
...
// authored in charly.yml never reaches the generator. Relocated from
// charly/config_test.go with mergeBoxConfig (K1-proper).
...
// Relocated from charly/provider_kind_invoke_test.go with MergePluginKindsMap (K1-proper).
