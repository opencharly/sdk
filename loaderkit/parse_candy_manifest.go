...
//   - Multi-document stream → error (the candy manifest is not a deploy file).
...
func SingleCandyMappingNode(path string, data []byte) (*yaml.Node, error) {
	...
	if len(docs) > 1 {
		return nil, fmt.Errorf("%s: the candy manifest is not a multi-document stream; deploy files belong in the unified charly.yml", path)
	}
