...
// materialize_wire.go — the serialization contract for a materialized UnifiedFile crossing the
// loader-materialize / loader-*-validate reverse legs (K1-LOADER RELOCATION, Unit B/D). It exists
// because UnifiedFile.PluginKinds is tagged `json:"-"` ("Host-internal — never serialized"): a plain
// json.Marshal of a UnifiedFile SILENTLY DROPS every standalone-template + plugin-kind entity
...
// ONE UnifiedFile in place, no serialization). But the PLUGIN-side witness (execLoaderExecutor) round-
// trips the UnifiedFile through HostBuild's []byte payload, so without this helper the plugin-side uf
...
// #48); UnifiedFile is itself a loaderkit hand-type, so its serialization envelope lives here too.
...
// materializedEnvelope carries a UnifiedFile plus the PluginKinds maps a plain json.Marshal drops.
...
func capturePluginKinds(uf *UnifiedFile, prefix string, out pluginKindsByPath) {
	if uf == nil {
		return
	}
...
func restorePluginKinds(uf *UnifiedFile, prefix string, in pluginKindsByPath) {
	if uf == nil {
		return
	}
...
// MarshalMaterialized serializes a materialized UnifiedFile for a reverse-leg []byte payload,
// PRESERVING PluginKinds (which a plain json.Marshal drops). Use it on EVERY loader leg that sends a
// materialized UnifiedFile across the wire (the loader-materialize reply + the loader-*-validate
// requests).
func MarshalMaterialized(uf *UnifiedFile) ([]byte, error) {
	env := materializedEnvelope{UF: uf, PluginKinds: pluginKindsByPath{}}
	capturePluginKinds(uf, "", env.PluginKinds)
	return json.Marshal(env)
}

// UnmarshalMaterialized reconstructs a materialized UnifiedFile from a MarshalMaterialized payload
// INTO uf, re-attaching PluginKinds at every namespace level so the result is byte-identical to the
// source. uf must be non-nil (the leg's own `merged`/scratch UnifiedFile).
func UnmarshalMaterialized(data []byte, uf *UnifiedFile) error {
