...
// resolves a device (charly deploy add android:, charly check run, charly box validate, …) sees the
...
func ValidatePreemptibleOnNode(name string, node *spec.DeployNode, d *spec.Diagnostics) {
	if node == nil {
		return
	}
...
//     StampDeployDescents runs before this validator in LoadUnified), never by switching on the
...
func preemptNeedsGPUResource(cnode *spec.DeployNode, resources map[string]*spec.ResolvedResource) bool {
	if cnode == nil {
		return false
	}
