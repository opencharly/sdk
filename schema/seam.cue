...
#CheckBedRequest: {
	op!:  string @go(Op)  // members-up | members-down | wait-ready | persist-overrides | gpu-prereq | libvirt-start | lease-acquire | lease-release | repo-override-pair
	bed!: string @go(Bed) // the disposable bundle (bed) name
	node?:      #Deploy @go(Node, optional=nillable) // the resolved bed node — every op but libvirt-start/repo-override-pair needs it
	image_tag?: string  @go(ImageTag)                // members-up: the per-run bed-scoped image tag (bringUpMembers's imageTag arg)
	active?:    bool    @go(Active)                  // lease-release: was the lease actually active (Release()/ReleaseFailed() no-op otherwise)
	ok?:        bool    @go(OK)                      // lease-release: true → Lease.Release, false → Lease.ReleaseFailed
}
...
#CheckBedReply: {
	lease_active?:       bool   @go(LeaseActive)
	repo_override_pair?: string @go(RepoOverridePair) // repo-override-pair's output ("" ⇒ not a submodule / no override needed)
